...
```dataviewjs
const pages = dv.pages('"world_building/wiki/locations"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "location");
dv.table(["location", "Total"], [["location", pages.length]]);
```
