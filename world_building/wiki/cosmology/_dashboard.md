...
```dataviewjs
const pages = dv.pages('"world_building/wiki/cosmology"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "cosmology");
dv.table(["cosmology", "Total"], [["cosmology", pages.length]]);
```
