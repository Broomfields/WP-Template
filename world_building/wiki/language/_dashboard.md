...
```dataviewjs
const pages = dv.pages('"world_building/wiki/language"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "language");
dv.table(["language", "Total"], [["language", pages.length]]);
```
