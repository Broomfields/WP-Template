...
```dataviewjs
const pages = dv.pages('"world_building/wiki/races"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "race");
dv.table(["race", "Total"], [["race", pages.length]]);
```
...
### Missing `biological_class`

```dataviewjs
const po = {high: 0, medium: 1, low: 2};
const pages = [...dv.pages('"world_building/wiki/races"')
  .where(p => (p.biological_class == null || p.biological_class === "") && !p.file.frontmatter["_author_only"])]
  .sort((a, b) => (po[a.file.frontmatter["_priority"]] ?? 99) - (po[b.file.frontmatter["_priority"]] ?? 99));
dv.table(["Type", "_Status", "Priority", "Entry"],
  pages.map(p => [p.type, p.file.frontmatter["_status"], p.file.frontmatter["_priority"], p.file.link]));
```
