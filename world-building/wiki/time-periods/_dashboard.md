---
title: "Time Periods Dashboard"
_author_only: true
---

# Time Periods Dashboard
...
```dataviewjs
const pages = dv.pages('"world-building/wiki/time-periods"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "time-period");
dv.table(["time-period", "Total"], [["time-period", pages.length]]);
```
...
### Missing `world_date_start`

```dataviewjs
const po = {high: 0, medium: 1, low: 2};
const pages = [...dv.pages('"world-building/wiki/time-periods"')
  .where(p => (p.world_date_start == null || p.world_date_start === "") && !p.file.frontmatter["_author_only"])]
  .sort((a, b) => (po[a.file.frontmatter["_priority"]] ?? 99) - (po[b.file.frontmatter["_priority"]] ?? 99));
dv.table(["Type", "_Status", "Priority", "Entry"],
  pages.map(p => [p.type, p.file.frontmatter["_status"], p.file.frontmatter["_priority"], p.file.link]));
```
