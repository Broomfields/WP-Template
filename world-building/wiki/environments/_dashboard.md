...
```dataviewjs
const pages = dv.pages('"world-building/wiki/environments"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "environment");
dv.table(["environment", "Total"], [["environment", pages.length]]);
```
