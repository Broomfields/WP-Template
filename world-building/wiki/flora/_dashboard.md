...
```dataviewjs
const pages = dv.pages('"world-building/wiki/flora"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "flora");
dv.table(["flora", "Total"], [["flora", pages.length]]);
```
