...
```dataviewjs
const pages = dv.pages('"world-building/wiki/characters"')
  .where(p => !p.file.frontmatter["_author_only"] && p.entity === "character");
dv.table(["character", "Total"], [["character", pages.length]]);
```
